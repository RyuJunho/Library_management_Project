...
class Panda:
    ...
    def book_rent(self, check_Phone, rent_ISBN, rent_date, return_date):  # 도서 대여
        rent_df = self.Book_df[(self.Book_df["ISBN"] == int(rent_ISBN))]
        if (self.user_rent_df['ISBN'] == int(rent_ISBN)).any():  # 대여하고자 하는 도서가 대여 중인 경우
            print("대여 중인 도서입니다.\n")
        elif (rent_df['ISBN'] != int(rent_ISBN)).any():
            print("존재하지 않는 도서입니다.\n")
        else:
            print("대여 완료 됐습니다.\n")
