...
class Panda:
    ...
    def book_search(self, combo):  # 도서 검색
        if combo == "제목":  # 콤보 박스를 제목으로 선택했을 때
            ...
            B_search_np = np.array(self.Book_df.loc[self.Book_df['제목'].str.contains(data_Tsearch), ['제목', '저자', 'ISBN',
                                                                                                    '대여여부']])  # 제목 데이터가 일부분이라도 포함되어 있으면 제목, 저자, ISBN, 대여여부 순으로 출력
            return B_search_np
        elif combo == "저자":  # 콤보 박스를 저자로 선택했을 때
            data_Asearch = input("저자를 입력하세요 : ")  # 저자 데이터
            B_search_np = np.array(self.Book_df.loc[self.Book_df['저자'].str.contains(data_Asearch), ['제목', '저자', 'ISBN',
    ...
    def book_delete(self):  # 도서 삭제
        ...
        if (del_df['대여여부'] == False).any():  # 대여를 하고 있는 경우
            ...
            self.Book_df = self.Book_df.drop(
                index=self.Book_df.loc[self.Book_df.ISBN == del_ISBN].index)  # 해당 ISBN을 가진 데이터프레임의 인덱스(행)를 삭제
        ...
        if (phone_df['전화번호'] == check_Phone).any():  # 동일한 전화번호가 하나라도 있는지 확인
            ...
            if (phone_df['ISBN'] == return_ISBN).any():  # 반납하고자 하는 도서 ISBN이 동일한 경우
                if (phone_df['대여여부'] == False).any():  # 대여 중인 경우
                    ...
                    self.user_rent_df.to_csv('Book_rent.csv', encoding='utf-8',
                                             index=False)  # 삭제 후 데이터를 확인하기 위한 반납 csv 파일 생성
            else:
                print("대여중인 도서가 아닙니다.\n")
        else:
