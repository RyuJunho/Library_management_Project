...
class Panda:
    def __init__(self, book, user, rent):   # 클래스 시작 항상 실행, 객체 생성 시 자동 호출
        self.Book_df = pd.read_csv(book).sort_values(by=['제목'], axis=0) # 데이터 프레임을 공유하기 위한 자신을 참조하는 매개변수 self로 데이터 프레임 생성
    ...
    def book_append(self):  # 도서 추가
        ...
        if (self.Book_df['ISBN'] == check_ISBN).any():    # 동일한 ISBN이 하나라도 있는지 판별
            ...
            data_to_insert = ([{'ISBN': check_ISBN, '제목': title, '저자': author, '출판사': pub, '가격': price, '관련링크': link, '도서설명': explanation, '대여여부': False}])    # 입력한 데이터를 딕셔너리로 생성
    ...
    def book_modify(self):  # 도서 수정
        ...
        if (self.Book_df['ISBN'] == check_ISBN).any():  # 동일한 ISBN 하나라도 있는지 확인
            ...
            self.Book_df.loc[self.Book_df.ISBN == check_ISBN, ('제목', '저자', '출판사', '가격', '관련링크', '도서설명')] = (title, author, pub, price, link, explanation)  # ISBN을 기준으로 수정하고자 하는 컬럼의 데이터 수정
        ...
        if (phone_df['전화번호'] == check_Phone).any():    # 동일한 전화번호가 하나라도 있는지 확인
            ...
            if (phone_df['ISBN'] == return_ISBN).any():  # 반납하고자 하는 도서 ISBN이 동일한 경우
                if (phone_df['대여여부'] == False).any(): # 대여 중인 경우
                    ...
                    self.user_rent_df.loc[self.user_rent_df.ISBN == return_ISBN, ('대여여부','대여일', '반납예정일')] = (True, None, None)  # ISBN을 기준으로 수정하고자 하는 컬럼의 데이터 수정
                    self.user_rent_df.to_csv('Book_rent.csv', encoding='utf-8', index=False)  # 삭제 후 데이터를 확인하기 위한 반납 csv 파일 생성
            else:
                print("대여중인 도서가 아닙니다.\n")
        else:
...
''' 클래스 코드 확인 용
book_Srh_Pan = Panda('Book_list.csv', 'user_list.csv','Book_BasicRent.csv')
book_Srh_Pan.book_search("제목")

book_App_Pan = Panda('Book_list.csv', 'user_list.csv','Book_BasicRent.csv')
book_App_Pan.book_append()

book_mod_Pan = Panda('Book_list.csv', 'user_list.csv','Book_BasicRent.csv')
book_mod_Pan.book_modify()

book_delete = Panda('Book_list.csv', 'user_list.csv','Book_BasicRent.csv')
book_delete.book_delete()

book_rent_Pan = Panda('Book_list.csv', 'user_list.csv','Book_BasicRent.csv')
book_rent_Pan.book_rent()

book_return_Pan = Panda('Book_list.csv', 'user_list.csv','Book_BasicRent.csv')
book_return_Pan.book_return()
'''
